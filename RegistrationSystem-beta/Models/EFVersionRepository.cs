...
namespace RegistrationSystem.Models
{
    public class EFVersionRepository : IVersionRepository
    {
        ...
        public Version MoveFirst() => context.Versions.FirstOrDefault();

        public Version MovePrev(string current_sys_code, string current_ver_code)
        {
            var first_entry = context.Versions.FirstOrDefault();
            if ((string.IsNullOrWhiteSpace(current_sys_code) ||
                string.IsNullOrWhiteSpace(current_ver_code)) ||
                (string.Compare(first_entry.Ver_Code, current_sys_code) == 0 || 
                 string.Compare(first_entry.Ver_Sys_Code, current_ver_code) == 0))
                return first_entry;
        ...
        public Version MoveLast() => context.Versions.LastOrDefault();
        ...
        public Version MoveNext(string current_sys_code, string current_ver_code)
        {
            ...
            var last_entry = context.Versions.LastOrDefault();
            if (string.Compare(last_entry.Ver_Sys_Code + last_entry.Ver_Code, 
                               current_sys_code + current_ver_code) == 0)
                return context.Versions.LastOrDefault();
